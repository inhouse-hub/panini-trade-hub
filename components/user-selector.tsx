...
export function UserSelector() {
  const { users, activeUser, setActiveUser, addUser, updateUser, deleteUser, getStats } = useUser()
      ...
      <div className="flex flex-wrap gap-2">
        {users.map(user => {
                ...
                <div className="flex flex-wrap gap-1 max-w-[100px]">
                  {AVATAR_OPTIONS.map(avatar => (
                    <button
                      ...
                      className={cn(
                        'w-6 h-6 rounded-md flex items-center justify-center text-sm transition-colors',
          ...
          return (
            <div
              ...
              className={cn(
                'group relative flex items-center gap-2 px-3 py-2 rounded-xl border transition-all cursor-pointer',
                isActive 
                  ? 'bg-gold/10 border-gold/50 shadow-lg shadow-gold/10' 
                  : 'bg-card/50 border-border hover:border-gold/30 hover:bg-card'
              )}
            >
              ...
              <div className={cn(
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation()
                    startEditing(user.id, user.name, user.avatar)
                  }}
        ...
        {isAddingUser ? (
          <div className="flex items-center gap-2 p-2 bg-card border border-gold/50 rounded-xl">
            ...
            <button
              onClick={() => { setIsAddingUser(false); setNewName('') }}
              className="p-1.5 rounded-lg bg-muted text-muted-foreground hover:bg-muted/80 transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
